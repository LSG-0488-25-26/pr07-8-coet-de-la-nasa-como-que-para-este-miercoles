package com.example.umafacts.ui.theme

import androidx.compose.ui.graphics.Color

// Catppuccin Mocha (Dark Theme)
val Rosewater = Color(0xFFF5E0DC)
val Flamingo = Color(0xFFF2CDCD)
val Pink = Color(0xFFF5C2E7)
val Mauve = Color(0xFFCBA6F7)
val Red = Color(0xFFF38BA8)
val Maroon = Color(0xFFEBA0AC)
val Peach = Color(0xFFFAB387)
val Yellow = Color(0xFFF9E2AF)
val Green = Color(0xFFA6E3A1)
val Teal = Color(0xFF94E2D5)
val Sky = Color(0xFF89DCEB)
val Sapphire = Color(0xFF74C7EC)
val Blue = Color(0xFF89B4FA)
val Lavender = Color(0xFFB4BEFE)

val Text = Color(0xFFCDD6F4)
val Subtext1 = Color(0xFFBAC2DE)
val Subtext0 = Color(0xFFA6ADC8)
val Overlay2 = Color(0xFF9399B2)
val Overlay1 = Color(0xFF7F849C)
val Overlay0 = Color(0xFF6C7086)
val Surface2 = Color(0xFF585B70)
val Surface1 = Color(0xFF45475A)
val Surface0 = Color(0xFF313244)
val Base = Color(0xFF1E1E2E)
val Mantle = Color(0xFF181825)
val Crust = Color(0xFF11111B)

// Catppuccin Latte (Light Theme)
val RosewaterLight = Color(0xFFDC8A78)
val FlamingoLight = Color(0xFFDD7878)
val PinkLight = Color(0xFFEA76CB)
val MauveLight = Color(0xFF8839EF)
val RedLight = Color(0xFFD20F39)
val MaroonLight = Color(0xFFE64553)
val PeachLight = Color(0xFFFE640B)
val YellowLight = Color(0xFFDF8E1D)
val GreenLight = Color(0xFF40A02B)
val TealLight = Color(0xFF179299)
val SkyLight = Color(0xFF04A5E5)
val SapphireLight = Color(0xFF209FB5)
val BlueLight = Color(0xFF1E66F5)
val LavenderLight = Color(0xFF7287FD)

val TextLight = Color(0xFF4C4F69)
val Subtext1Light = Color(0xFF5C5F77)
val Subtext0Light = Color(0xFF6C6F85)
val Overlay2Light = Color(0xFF7C7F93)
val Overlay1Light = Color(0xFF8C8FA1)
val Overlay0Light = Color(0xFF9CA0B0)
val Surface2Light = Color(0xFFACB0BE)
val Surface1Light = Color(0xFFBCC0CC)
val Surface0Light = Color(0xFFCCD0DA)
val BaseLight = Color(0xFFEFF1F5)
val MantleLight = Color(0xFFE6E9EF)
val CrustLight = Color(0xFFDCE0E8)